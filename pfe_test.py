...
# Mock the NAO proxies
class MockTTS:
    def say(self, text):
        print(f"(TTS) NAO says: {text}")

class MockMotion:
    def openHand(self, hand):
        print(f"(Gesture) NAO opens {hand} hand")
        
    def closeHand(self, hand):
        print(f"(Gesture) NAO closes {hand} hand")
...
def enact_frame(frame):
    ...
    if gesture == "Wave":
        ...
        print("(Gesture) NAO simulates breathing")
...
for frame in educational_frames:
    ...
    # Ask if children have any questions
    tts.say("Do you have any questions?")
